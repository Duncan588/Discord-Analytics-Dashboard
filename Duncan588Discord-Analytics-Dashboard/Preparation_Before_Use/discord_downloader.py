...
from shared.discord_api import default_downloader_token, discord_headers, send_direct_message
...
class BotCapabilityError(RuntimeError):
    """The configured account cannot perform the requested DCE export."""
...
def get_token(bot_id=None):
    ...
    row = None
    if bot_id:
        row = conn.execute("SELECT name,token,token_type FROM download_bots WHERE id=?", (bot_id,)).fetchone()
    default_token = default_downloader_token()
    conn.close()
    if row and row["token"]:
        return {"name": row["name"], "token": row["token"], "token_type": row["token_type"] or "bot"}
    if default_token:
        return {"name": "env-default", "token": default_token, "token_type": "bot"}
    return None
...
def get_server_bots(config_id=None, guild_id=None):
    ...
    if config_id:
        ...
        cfg=None
    if cfg:
        rows=conn.execute("SELECT b.id,b.name,b.token,b.token_type FROM download_config_bots cb JOIN download_bots b ON b.id=cb.bot_id WHERE cb.config_id=? ORDER BY b.id",(cfg["id"],)).fetchall()
        result=[dict(r) for r in rows]
        if cfg["use_default_bot"]:
            default=get_token()
            if default: result.append({"id":None,"name":"默认下载机器人","token":default["token"],"token_type":"bot"})
...
def _login_token(token, token_type="bot"):
    """Validate either token kind without logging the credential."""
    ...
    return str(response.json().get("id") or "unknown")
...
def authenticate_download_bots(task_id, bots):
    ...
    def login_one(bot):
        try:
            user_id = _login_token(bot["token"], bot.get("token_type", "bot"))
            log_step("任务 #%s | 下载账号登录成功 | name=%s type=%s discord_id=%s", task_id, bot["name"], bot.get("token_type", "bot"), user_id)
            return bot
        except Exception as exc:
            log_step("任务 #%s | 下载账号登录失败 | name=%s type=%s | error=%s", task_id, bot["name"], bot.get("token_type", "bot"), exc)
            return None
...
def update_task(task_id, **values):
    ...
    conn = db()
    values = dict(values)
    current = conn.execute("SELECT * FROM download_tasks WHERE id=?", (task_id,)).fetchone()
    if current:
        next_status = values.get("status", current["status"])
        current_status = current["status"]

        # elapsed_seconds/estimated_seconds/speed 是服务端派生字段。调用方只
        # 更新业务进度，不能把 time.time()-started 之类的墙上时间写回来，
        # 否则 active_started_at 会再次加上同一段时间，造成重复累计。
        for key in ("elapsed_seconds", "estimated_seconds", "speed"):
            values.pop(key, None)

        # 运行计时只从 active_started_at 开始；排队和暂停时间不会混入耗时。
        # 新任务第一次进入 running 时建立活动区间的起点。
        if next_status == "running" and values.get("phase") != "queued" and not current["active_started_at"] and not values.get("active_started_at"):
            values["active_started_at"] = now()
        merged = dict(current)
        merged.update(values)
        # merged 中必须使用数据库已经结算过的累计值，而不是调用方带来的
        # 旧/墙上时间。这样即使多个 Worker 高频更新，也不会重复加时。
        merged["elapsed_seconds"] = current["elapsed_seconds"]
        if next_status == "running":
            calculated_at = datetime.now(timezone.utc)
            timing = calculate_task_timing(merged, calculated_at)
            values.update(timing)
            # 已结算的时间从新的活动起点继续，避免下一次更新重复累计同一段时间。
            values["active_started_at"] = calculated_at.isoformat()
        elif next_status in ("paused", "completed", "failed", "cancelled") and current_status == "running":
            # 暂停和终态更新也要把最后一段运行时间计入累计值。
            timing_input = dict(merged)
            timing_input["status"] = "running"
            timing = calculate_task_timing(timing_input, datetime.now(timezone.utc))
            values.update(timing)
            values["active_started_at"] = None
...
def _rest_json(path, token, token_type="bot", params=None):
    """GET a Discord REST resource with bounded retries for transient errors."""
    for attempt in range(6):
        ...
        if response.status_code == 429:
            try:
                wait = max(1.0, min(90.0, float(response.json().get("retry_after", 1))))
            except (ValueError, TypeError):
                wait = 1.0
            time.sleep(wait)
            continue
        if response.status_code in (500, 502, 503, 504) and attempt < 5:
            time.sleep(min(30.0, 2.0 ** attempt))
            continue
        response.raise_for_status()
        return response.json()
    raise RuntimeError(f"Discord REST 请求重试次数超限：{path}")


def _rest_thread_row(thread):
    thread_id = int(thread["id"])
    created_at = discord.utils.snowflake_time(thread_id)
    last_active = thread.get("last_message_id")
    if last_active:
        try:
            last_active = discord.utils.snowflake_time(int(last_active)).isoformat()
        except (TypeError, ValueError):
            last_active = None
    return {
        "id": thread_id,
        "name": thread.get("name") or "thread",
        "created_at": created_at.isoformat(),
        "last_active_at": last_active or created_at.isoformat(),
        "archived": bool(thread.get("thread_metadata", {}).get("archived", False)),
        "locked": bool(thread.get("thread_metadata", {}).get("locked", False)),
...
def enumerate_server_rest(guild_id, forum_id, token, task_id, token_type="user", progress_cb=None, row_cb=None, scan_before=None, cursor_cb=None, initial_seen=None):
    """Scan Forum threads without Gateway so raw user tokens are supported."""
    seen = {int(x) for x in (initial_seen or ())}
    processed = 0

    def consume(threads):
        nonlocal processed
        for thread in threads:
            if str(thread.get("parent_id")) != str(forum_id) or int(thread["id"]) in seen:
                continue
            seen.add(int(thread["id"]))
            row = _rest_thread_row(thread)
            if row_cb:
                row_cb(row)
            processed += 1
            if progress_cb and (processed == 1 or processed % 50 == 0):
                progress_cb(processed)

    active = _rest_json(f"/guilds/{guild_id}/threads/active", token, token_type)
    consume(active.get("threads") or [])
    log_step("任务 #%s | REST Forum 活跃帖子扫描完成 | Forum=%s", task_id, forum_id)

    before = None
    if scan_before:
        try:
            before = datetime.fromisoformat(str(scan_before).replace("Z", "+00:00"))
            if before.tzinfo is None:
                before = before.replace(tzinfo=timezone.utc)
        except (TypeError, ValueError):
            log_step("任务 #%s | REST 扫描游标无效，将从历史起点开始 | cursor=%s", task_id, scan_before)
    ...
    while True:
        ...
        payload = _rest_json(f"/channels/{forum_id}/threads/archived/public", token, token_type, params)
        raw_page = payload.get("threads") or []
        if not raw_page:
            break
        page += 1
        consume(raw_page)
        timestamps = []
        for thread in raw_page:
            raw_timestamp = thread.get("archive_timestamp")
            if raw_timestamp:
                try:
                    ...
                    pass
            if not timestamps:
                timestamps.append(discord.utils.snowflake_time(int(thread["id"])))
        oldest = min(timestamps)
        next_before = oldest - timedelta(microseconds=1)
        if before is not None and next_before >= before:
            raise RuntimeError(f"REST 历史扫描分页游标未向前移动：before={before.isoformat()}")
        before = next_before
        if cursor_cb:
            cursor_cb(before.isoformat())
        log_step("任务 #%s | REST 历史扫描第 %s 页完成 | API本页=%s | 累计=%s", task_id, page, len(raw_page), processed)
        if not payload.get("has_more"):
            break
    log_step("任务 #%s | REST Forum 历史扫描结束 | page=%s | 累计=%s", task_id, page, processed)
    return []


def enumerate_server(guild_id, forum_id, token, task_id, lower_id=0, upper_id=None, progress_cb=None, row_cb=None, scan_before=None, cursor_cb=None, initial_seen=None, token_type="bot"):
    if str(token_type or "bot").lower() == "user":
        return enumerate_server_rest(guild_id, forum_id, token, task_id, token_type, progress_cb, row_cb, scan_before, cursor_cb, initial_seen)
...
def _export_batch(dce, token, tids, output_template, task_id, bot_name=""):
    """用一个 DCE 进程批量导出帖子，并按帖子 ID 分别写出 JSON 文件。"""
    tids = [str(tid) for tid in tids]
    if not tids:
        return
    log_step(
        "任务 #%s | 机器人=%s | 开始批量 DCE 下载 | 数量=%s | 帖子范围=%s..%s",
    ...
    dce_log_path = Path(output_template).parent / f".dce-{os.getpid()}-{time.time_ns()}.log"
    dce_log = dce_log_path.open("w", encoding="utf-8", errors="replace")
    proc = subprocess.Popen(
        cmd, stdout=dce_log, stderr=subprocess.STDOUT,
        cwd=BASE_DIR, env=os.environ.copy(), start_new_session=True
    )
    log_step("任务 #%s | 机器人=%s | 批量 DCE PID=%s 已启动 | 数量=%s", task_id, bot_name, proc.pid, len(tids))
    outputs = {tid: Path(str(output_template).replace("%c", tid)) for tid in tids}
    try:
        try:
            while proc.poll() is None:
                state = task_status(task_id)
                if state == "paused":
                    log_step("任务 #%s | 机器人=%s | 检测到暂停，终止批量 DCE PID=%s | 数量=%s", task_id, bot_name, proc.pid, len(tids))
                    proc.terminate()
                    try:
                        proc.wait(timeout=3)
                    ...
                    raise TaskPaused()
                if state == "cancelled":
                    log_step("任务 #%s | 机器人=%s | 检测到取消，终止批量 DCE PID=%s | 数量=%s", task_id, bot_name, proc.pid, len(tids))
                    proc.terminate()
                    try:
                        proc.wait(timeout=3)
                    except subprocess.TimeoutExpired:
        ...
        dce_log.close()
    produced = [tid for tid, out in outputs.items() if out.exists() and out.stat().st_size > 0]
    missing = [tid for tid in tids if tid not in produced]
    log_step(
        "任务 #%s | 机器人=%s | 批量 DCE 完成 | 数量=%s | 成功文件=%s | 缺失=%s | returncode=%s",
        task_id, bot_name, len(tids), len(produced), len(missing), proc.returncode,
    )
    if proc.returncode != 0:
        try:
            diagnostic = dce_log_path.read_text(encoding="utf-8", errors="replace").replace(token, "<TOKEN>")
        except OSError:
            diagnostic = ""
        summary_lines = [line.strip() for line in diagnostic.splitlines() if line.strip()]
        summary = " | ".join(summary_lines[-8:])[-1800:]
        if "message_content privileged intent" in diagnostic.lower():
            log_step("任务 #%s | 机器人=%s | DCE 能力不可用 | 原因=%s", task_id, bot_name, summary)
            raise BotCapabilityError("DCE 账号缺少 MESSAGE_CONTENT privileged intent", outputs)
        if summary:
            log_step("任务 #%s | 机器人=%s | DCE 错误摘要=%s", task_id, bot_name, summary)
    if proc.returncode == 0 and dce_log_path.exists():
        dce_log_path.unlink(missing_ok=True)
    # DCE 可能在批次内成功写出部分文件，即使最终返回码非 0；调用方会逐帖
    # 导入成功文件，并只重试缺失文件，避免整批重复导入。
    return outputs
...
def dce_batch_size():
    """返回单个 DCE 进程处理的帖子数；批次保持在可控范围内。"""
    try:
        value = int(os.getenv("DCE_BATCH_SIZE", "5"))
    except (TypeError, ValueError):
        value = 5
...
def run_task(task, dce):
    ...
    # 中央任务队列：每个 Worker 一次拿一个小批次帖子，并使用自己的 Token
    # 启动一个 DCE 进程。批次内仍按帖子 ID 分文件，导入链路无需变化。
    q=Queue()
    workers=[]
    ...
    attempted_bot_keys={bot_key(bot) for bot in bots}
    disabled_bot_keys=set()

    def bot_snapshot():
        with bot_lock:
            return list(bots)

    def disable_bot(bot, reason):
        """Remove an account that DCE proved incapable of exporting messages."""
        key=bot_key(bot)
        with bot_lock:
            if key in disabled_bot_keys:
                return
            disabled_bot_keys.add(key)
            bots[:]=[candidate for candidate in bots if bot_key(candidate) != key]
        update_task(task_id,active_bots=active_worker_count(),message=f'下载机器人 {bot["name"]} 暂停使用：{reason}')
        log_step("任务 #%s | 已停用下载机器人 | name=%s | reason=%s | 剩余机器人=%s", task_id, bot["name"], reason, len(bot_snapshot()))

    def retry_or_fail(row, exc, bot):
        tid=str(row['id'])
        retry_counts[tid]=retry_counts.get(tid,0)+1
        if retry_counts[tid] < 2 and task_status(task_id) not in ('paused','cancelled'):
            q.put(row)
            log_step("任务 #%s | 机器人=%s | 帖子=%s | 批量下载失败，重新加入队列 | retry=%s | error=%s", task_id, bot['name'], tid, retry_counts[tid], exc)
        else:
            mark_item(task_id,tid,status='failed',bot_name=bot['name'],error=str(exc))
            failed_posts.append(f'帖子 {tid}: {exc}')

    def import_downloaded_row(row, out, bot):
        tid=str(row['id'])
        if not out.exists() or out.stat().st_size == 0:
            raise RuntimeError(f"帖子 {tid} 下载失败，批量 DCE 未生成文件")
        ...
        # 删除请求可能与导入同时发生。导入完成后再次检查，防止取消任务的
        # 帖子在 Web 端清理之后被写回分析库。
        if task_status(task_id) != 'running':
        ...
        try:
            while not stop_event.is_set():
                ...
                if state in ('paused','cancelled'):
                    return
                if bot_key(bot) in disabled_bot_keys:
                    return
                batch=[]
                try:
                    ...
                    if scan_finished.is_set() and q.unfinished_tasks == 0:
                        return
                    continue
                for _ in range(dce_batch_size() - 1):
                    try:
                ...
                try:
                    for row in batch:
                        ...
                        if not st or st[0] not in ('downloaded','skipped'):
                            eligible.append(row)
                    if not eligible:
                        continue
                    tids=[str(row['id']) for row in eligible]
                    log_step("任务 #%s | 机器人=%s | 取出批次 | 数量=%s | 首帖=%s | 剩余队列=%s", task_id, bot["name"], len(tids), tids[0], q.qsize())
                    if task_status(task_id) != 'running':
                        raise TaskCancelled()
                    output_template=backup/'%c.json'
                    outputs={tid:backup/f'{tid}.json' for tid in tids}
                    for out in outputs.values():
                        # 上一次批次被中断后留下的临时文件不能被误当作本次结果。
                        out.unlink(missing_ok=True)
                    batch_error=None
                    try:
                        _export_batch(dce,bot['token'],tids,output_template,task_id,bot["name"])
                    except (TaskPaused,TaskCancelled):
                        raise
                    except BotCapabilityError as exc:
                        disable_bot(bot, str(exc))
                        batch_error=exc
                    except Exception as exc:
                        batch_error=exc
                    for row in eligible:
                        ...
                        out=outputs[tid]
                        try:
                            if isinstance(batch_error, BotCapabilityError) and out.exists() and out.stat().st_size > 0:
                                # DCE can export some channels before reporting a
                                # missing privileged intent. Keep those valid
                                # files and requeue only the missing channels.
                                import_downloaded_row(row,out,bot)
                            elif batch_error:
                                raise batch_error
                            else:
                                import_downloaded_row(row,out,bot)
    ...
    bot_refresh_stop=threading.Event()
    def refresh_download_bots():
        """读取配置新增的机器人；验证通过后立即加入当前任务的 Worker 池。"""
        try:
            configured=get_server_bots(config_id=cfg["id"] if cfg else None,guild_id=guild_id)
        except Exception as exc:
            log_step("任务 #%s | 刷新下载机器人配置失败 | error=%s", task_id, exc)
            return
        candidates=[]
        with bot_lock:
            for bot in configured:
                key=bot_key(bot)
                if key not in attempted_bot_keys:
                    attempted_bot_keys.add(key)
                    candidates.append(bot)
        if not candidates:
            return
        valid=authenticate_download_bots(task_id,candidates)
        if not valid:
            return
        with bot_lock:
            existing={bot_key(bot) for bot in bots}
            for bot in valid:
                if bot_key(bot) not in existing:
                    bots.append(bot)
                    existing.add(bot_key(bot))
        for bot in valid:
            launch_worker(bot)
        update_task(task_id,active_bots=active_worker_count(),message=f'已动态加入 {len(valid)} 个下载机器人，当前活跃 Worker {active_worker_count()}')
        log_step("任务 #%s | 动态加入下载机器人完成 | 新增=%s | 当前机器人=%s", task_id, ", ".join(bot["name"] for bot in valid), len(bot_snapshot()))

    def bot_monitor():
        while not bot_refresh_stop.wait(3):
            if task_status(task_id) != 'running':
                return
            refresh_download_bots()
    ...
    def scan_monitor():
        while not monitor_stop.wait(2):
            ...
            if scan_finished.is_set():
                return
            try:
                update_task(task_id, scan_processed=scan_processed, scan_discovered=total_items(), total=total_items(), completed=done_items(), active_bots=active_worker_count(), message=f'扫描中：已扫描 {scan_processed} · 已发现 {total_items()} · 下载队列 {q.qsize()} · 活跃下载机器人 {active_worker_count()}')
    ...
    try:
        update_task(task_id,status='running',phase='downloading' if resume else 'scanning' ,mode=mode,started_at=task['started_at'] or now(),scan_started_at=task['scan_started_at'] or now(),scan_bot_name=scan_bot['name'],active_bots=len(bot_snapshot()) if resume else len(other_bots),message='继续下载：跳过已完成帖子，只处理未完成帖子' if resume else ('更新模式：扫描帖子 ID 与最后活跃时间' if mode=='update' else '首次模式：扫描帖子 ID 与最后活跃时间'))

        threading.Thread(target=bot_monitor,name=f'bot-monitor-{task_id}',daemon=True).start()
        ...
        if resume:
            ...
            for bot in bot_snapshot():
                if bot_key(bot) == bot_key(scan_bot):
                    continue
                launch_worker(bot)

            def scanner_thread():
                nonlocal scanner_error
                try:
                    ...
                    enumerate_server(
                        ...
                        token_type=scan_bot.get("token_type", "bot")
                    )
                    ...
                    # 扫描客户端返回后必须可靠完成“扫描 Worker -> 下载 Worker”交接。
                    # 旧逻辑在交接异常时没有设置 scan_finished，导致任务永久停留在
                    # scanning，且扫描机器人从未开始下载；这里保证事件一定释放。
                    try:
                        ...
                        if state_after_scan not in ('paused','cancelled') and not scan_failed:
                            ...
                            update_task(
                                ...
                                total=total_items(), active_bots=active_worker_count(),
                                message=f'扫描完成：共发现 {total_items()} 个帖子，扫描机器人加入下载池'
                            )
                            log_step("任务 #%s | 扫描完成 | 扫描机器人已加入下载池 | 全部 Worker=%s", task_id, active_worker_count())
                    except Exception as exc:
                        scanner_error=str(exc)
                        failed_posts.append(f'扫描完成后交接失败: {exc}')
                        update_task(task_id,message=f'扫描完成，但扫描机器人加入下载池失败：{exc}')
                        log_step("任务 #%s | 扫描完成后 Worker 交接失败 | error=%s", task_id, exc)
                    finally:
                        scan_finished.set()
        ...
        if resume and not workers:
            for bot in bot_snapshot():
                launch_worker(bot)
            # scan_monitor 可能在 Worker 启动前完成最后一次心跳，导致
            # 页面暂时显示 active_bots=0。启动完成后立即写入真实数量。
            update_task(
                task_id,
                phase='downloading',
                active_bots=active_worker_count(),
                message=f'继续下载：已恢复未完成帖子，活跃下载机器人 {active_worker_count()} 个',
            )
            ...
            if scan_finished.is_set():
                if q.unfinished_tasks > 0:
                    for bot in bot_snapshot():
                        launch_worker(bot)
                    if not bot_snapshot():
                        raise RuntimeError('没有可用的下载机器人处理剩余帖子')
            else:
                for bot in bot_snapshot():
                    if bot_key(bot) == bot_key(scan_bot):
                        continue
        ...
        shutil.rmtree(task_root,ignore_errors=True)
        update_task(task_id,status='completed',phase='completed',completed=done,total=total,active_bots=0,message=f"{'更新' if mode=='update' else '首次下载'}完成：{done}/{total}，每个帖子下载后已实时写入数据库，共 {total_messages} 条消息",finished_at=now())
    ...
    while True:
        ...
        if slots > 0:
            ...
            for task in candidates:
                ...
                update_task(tid, status='running', phase='queued', active_started_at=None, message='已进入下载调度队列')

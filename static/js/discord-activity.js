...
(function () {
  ...
  var ACTIVITY_CONTEXT_KEY = "discordActivityContext";
  var ACTIVITY_CONTEXT_PARAMS = [
    "frame_id", "instance_id", "platform", "guild_id", "channel_id",
    "location_id", "mobile_app_version", "custom_id", "referrer_id",
  ];
  ...
  function activityContextFromSearch(search) {
    var source = new URLSearchParams(search || "");
    var context = new URLSearchParams();
    ACTIVITY_CONTEXT_PARAMS.forEach(function (name) {
  ...
  function restoreActivityContext() {
    ...
    var current = activityContextFromSearch(window.location.search);
    if (current.get("frame_id") && current.get("instance_id") && current.get("platform")) {
      sessionStorage.setItem(ACTIVITY_CONTEXT_KEY, current.toString());
      return;
    }

    var saved = new URLSearchParams(sessionStorage.getItem(ACTIVITY_CONTEXT_KEY) || "");
    if (!saved.get("frame_id") || !saved.get("instance_id") || !saved.get("platform")) return;
    ...
    if (changed) {
      // Server-rendered pages are allowed to navigate normally, but the SDK
      // needs Discord's context parameters on every page load. Replace only
      // the address bar; do not cause another request or a navigation loop.
      window.history.replaceState(window.history.state, document.title, url.pathname + url.search + url.hash);
    }
  }

  function addActivityContextToLink(link) {
    if (!link || !link.href || !isInsideDiscordActivity()) return;
    ...
    } catch (e) { /* malformed links are left untouched */ }
  }

  function preserveActivityContextOnLinks() {
    if (!isInsideDiscordActivity()) return;
    document.querySelectorAll("a[href]").forEach(addActivityContextToLink);
    // Covers links created later by Alpine or other page scripts.
    document.addEventListener("click", function (event) {
      var link = event.target && event.target.closest ? event.target.closest("a[href]") : null;
      addActivityContextToLink(link);
    }, true);
  }
  ...
  function handleExternalLinkClick(event) {
    ...
    getLinkSdk().then(function (sdk) {
      ...
      // Never fall back to window.open/location in an Activity. Discord must
      // approve external navigation through OPEN_EXTERNAL_LINK; a browser
      // fallback is rejected by the Activity sandbox and can close the page.
      console.warn("[discord-activity] Discord 未批准外部链接，已阻止跳转：", err);
      showActivityError("Discord 客户端未批准此外部链接，已阻止跳转。请重新打开小活动后重试。", 0);
    });
  }
  ...
  preserveActivityContextOnLinks();
